...
class TrainingModel(nn.Module):
    ...
    def run_backbone(self, image_seqs):
        ...
        for i in range(0,images_tensor.size(0)):
          img = torch.permute(images_tensor[i],(1,2,0)).detach().cpu().numpy()
         
          img = cv2.normalize(img, None, 0, 255, cv2.NORM_MINMAX).astype('uint8')
          im = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
          z = torch.permute(torch.tensor(im),(2,0,1)).unsqueeze(0)
          if i==0:
            images_tensorx = z
          else:
            images_tensorx = torch.cat((images_tensorx,z),dim=0)
          #print(z.shape)
          #plt.imshow(im)
        images_tensorx = images_tensorx.to(device='cuda:0').float()
        #print('ttt',images_tensorx.size())
        images_tensor = images_tensorx;
