...
class SqueezeExpandDecoder(nn.Module):
    def __init__(self, in_channels, num_classes, inter_channels, feature_scales, foreground_channel=False,
        ...
        self.fskey_conv = nn.Conv2d(inter_channels[3],inter_channels[3]//8,3,padding='same')
        self.fsvalue_conv = nn.Conv2d(inter_channels[3],inter_channels[3]//8,3,padding='same')
        self.fckey_conv = nn.Conv2d(inter_channels[3],inter_channels[3]//8,3,padding='same')
        self.softmax_attn = nn.Softmax(dim=1)
        self.fckey_conv = nn.Conv2d(inter_channels[3],inter_channels[3]//8,3,padding='same')
        self.fA_conv  = nn.Conv2d(inter_channels[3]//8,inter_channels[3],1,padding='same')
