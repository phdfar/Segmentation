...
class TrainingModel(nn.Module):
    ...
    def run_backbone(self, image_seqs):
        ...
        #print(images_tensor.size(0))
        mag=[];image1=[];image2=[]
        for i in range(0,images_tensor.size(0)):
          img1 = torch.permute(images_tensor[i],(1,2,0)).detach().cpu().numpy()
          try:
            img2 = torch.permute(images_tensor[i+1],(1,2,0)).detach().cpu().numpy()
          except:
            img2 = torch.permute(images_tensor[i-1],(1,2,0)).detach().cpu().numpy()
          ...
          try:
            ...
            pass
          if flag!=0:
            a = opt.getopticalflow_gray(im_out,gray2)
          else:
            a = opt.getopticalflow_gray(gray1,gray2)

          #ang.append(b); 
          mag.append(a);
          #img1 = torch.permute(torch.tensor(np.asarray(img1)).to(device='cuda:0'),(2,0,1)).unsqueeze(0);
          #img2 = torch.permute(torch.tensor(np.asarray(img2)).to(device='cuda:0'),(2,0,1)).unsqueeze(0);
          image1.append(img1)
          image2.append(img2)

          """
          cv2.imwrite('/content/'+str(i)+'x.png',img1)
          cv2.imwrite('/content/'+str(i+1)+'x.png',img2)

          cv2.imwrite('/content/'+str(i)+'mag.png',a*255)
          cv2.imwrite('/content/'+str(i)+'ang.png',b*255)
          """

        mag = torch.tensor(np.asarray(mag)).to(device='cuda:0').unsqueeze(0)
        ...
        z = OrderedDict([(k, v) for k, v in zip(self.feature_map_scales, features)])
        ...
        title=['ang','mag']
        data = [ang,mag]
        z = OrderedDict([(k, v) for k, v in zip(title, data)])

        OrderedDict
        #print('self.feature_map_scales',self.z)
        for k in z:
          print('k',k,z[k].size())
        print(asd)
        """
    ...
    def forward_embeddings_and_semseg(self, features, num_seqs, num_frames):
        ...
        embedding_head_input_features.append(features['mag'])
        embedding_head_input_features.append(features['img1'])
        embedding_head_input_features.append(features['img2'])
