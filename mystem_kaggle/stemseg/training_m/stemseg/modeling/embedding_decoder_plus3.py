...
class SqueezingExpandDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        feat_map_32x, feat_map_16x, feat_map_8x, feat_map_4x, opt,img1,img2 = x
        
        #opt = opt.unsqueeze(0).float()
        opt = torch.permute(opt,(0,4,1,2,3)).float()
        ...
        optx = self.convop1(opt)
        optx = self.convop2(optx)

        optx = torch.cat((optx,img1x,img2x),dim=1)
