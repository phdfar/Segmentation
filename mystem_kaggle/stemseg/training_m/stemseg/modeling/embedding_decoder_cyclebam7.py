...
class SqueezingExpandDecoder(nn.Module):
    def __init__(self, in_channels, inter_channels, embedding_size, tanh_activation,
        ...
        self.keyconv1 = nn.Conv3d(32, 1, 1, bias=False)
        self.keyconv2 = nn.Conv3d(16, 8, 1, bias=False)
    ...
    def forward(self, x):
        ...
        z = feat_map_4x
        f4t = torch.permute(feat_map_4x, (0, 2, 1, 3, 4))
        avg_pool = F.avg_pool3d(f4t, (z.size(2),z.size(3), z.size(4)), stride=(z.size(2),z.size(3), z.size(4)) )
        max_pool = F.max_pool3d(f4t, (z.size(2),z.size(3), z.size(4)), stride=(z.size(2),z.size(3), z.size(4)) )
        fx = torch.cat((avg_pool,max_pool), dim=1)
        fx = torch.permute(fx, (0, 2, 1, 3, 4))
        fx = self.keyconv1(fx)
        fx = torch.permute(fx, (0, 2, 1, 3, 4))
        fx = self.keyconv2(fx)
        fx = fx.squeeze(-1).squeeze(-1).squeeze(-1)
        MC_F4  = F.sigmoid(fx)
