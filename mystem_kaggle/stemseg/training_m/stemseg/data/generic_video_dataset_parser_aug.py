...
def parse_generic_video_dataset(base_dir, dataset_json):
    ...
    seqs = [GenericVideoSequence(seq, base_dir) for seq in dataset["sequences"]]
    seqs_aug1 = [GenericVideoSequence(seq, base_dir,1) for seq in dataset["sequences"]] 
    seqs_aug2 = [GenericVideoSequence(seq, base_dir,2) for seq in dataset["sequences"]]  
    seqs = seqs + seqs_aug1 + seqs_aug2
    random.shuffle(seqs)
    #print(seqs)
...
class GenericVideoSequence(object):
    def __init__(self, seq_dict, base_dir,flag=0):
        ...
        if flag==0:    
            ...
            for path in seq_dict["image_paths"]:
                temp.append(path+'_aug1')
            self.image_paths = temp
        elif flag==2:
            temp = []
            for path in seq_dict["image_paths"]:
                temp.append(path+'_aug2')
            self.image_paths = temp
    ...
    def load_images(self, frame_idxes=None):
        ...
        for t in frame_idxes:
            ...
            if '_aug' not in self.image_paths[t]:
                ...
                if '_aug1' in self.image_paths[t]: 
                    pathaug = self.image_paths[t].replace('_aug1','')
                    im = cv2.imread(os.path.join(self.base_dir, pathaug), cv2.IMREAD_COLOR)
                    im = zoom_at(cv2.flip(im, 180),2,angle=0)
                    #print('_aug1')
                else:
                    index = random.randint(1,2)
                    pathaug = self.image_paths[t].replace('_aug2','')
                    if index==2:
                        im = cv2.imread(os.path.join(self.base_dir, pathaug), cv2.IMREAD_COLOR)
                        ...
                        im = cv2.filter2D(im, -1, kernel_motion_blur)
            ...
            for instance_id in self.instance_ids:
                if instance_id in self.segmentations[t]:
                    ...
                    if '_aug' not in self.image_paths[t]:
                        ...
                        if '_aug1' in self.image_paths[t]: 
                            im = np.ascontiguousarray(masktools.decode(rle_mask).astype(np.uint8))
                            im = zoom_at(cv2.flip(im, 180),2,angle=0)
                            masks_t.append(im)
                        else:
                            masks_t.append(np.ascontiguousarray(masktools.decode(rle_mask).astype(np.uint8)))
