# -*- coding: utf-8 -*-
"""
Created on Sat Apr  9 15:29:58 2022
...
class SqueezingExpandDecoder(nn.Module):
    def __init__(self, in_channels, inter_channels, embedding_size, tanh_activation,
        ...
        self.block_32x = nn.Sequential(
            ConvType(in_channels, inter_channels[0]-128, 3, stride=1, padding=1, dilation=1),
        ...
        self.block_16x = nn.Sequential(
            ConvType(in_channels, inter_channels[1]-128, 3, stride=1, padding=1),
            NormType(inter_channels[1]),
            nn.ReLU(inplace=True),
            UpsampleTrilinear3D(scale_factor=(1, 4, 4), align_corners=False),
        ...
        self.conv_new = nn.Conv3d(512, inter_channels[3], 1, bias=False)
