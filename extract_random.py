...
def st_feature(image):
    ...
    skewness = 3 * (mean_intensity - median_intensity) / std_deviation
    kurtosis = np.mean(((image - mean_intensity) / std_deviation) ** 4)
    
       
    hist = np.histogram(image.ravel(), bins=30)[0]
    entropy = calculate_entropy((hist+0.001)/len(image.ravel()))
    
    return mean_intensity,std_deviation,median_intensity,skewness,kurtosis,entropy
...
def extract_features(
    ...
    for i, (images, files, indices) in enumerate(pbar):
        ...
        desires=[];statics=[]
        for _ in range(0,10):
            index = np.random.choice(np.arange(0, output_dict['k'].shape[2]), size=5, replace=True)
            desire = output_dict['k'][:,:,index]
            x = desire[0].reshape(int(H//16),int(W//16),5).detach().cpu().numpy()
            
            stf={}
            for ch in range(x.shape[2]):
                y = x[:,:,ch]
                z = (y - np.min(y)) / (np.max(y)-np.min(y))
                mean_intensity,std_deviation,median_intensity,skewness,kurtosis,entropy = st_feature(z)
                stf.update({ch:{'mean':mean_intensity,'std':std_deviation,'median':median_intensity,'skewness':skewness,'kurtosis':kurtosis,'entropy':entropy}})
            
            desires.append({'feature':x})
            statics.append({'st_feature':stf,'index':index})
          
        output_dict['k']=desires;
        # Save
        accelerator.save(output_dict, str(output_file))
        accelerator.save(statics, str(output_file).replace('.pth','.obj'))
