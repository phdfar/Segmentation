...
def _extract_multi_region_segmentations(
    ...
    eig = np.load(feature_path)
    H_patch = eig.shape[0], W_patch=eig.shape[1]
    eig=eig.reshape((H_patch*W_patch,5))
    sp = feature_path.split('/'); id =sp[-1].replace('.pth.npy','');
    ...
    if kmeans_baseline:
        ...
        eigenvectors = eig[1:1+num_eigenvectors] 
...
def _extract_single_region_segmentations(
    ...
    H_patch = eig.shape[0];W_patch=eig.shape[1]
    eig=eig.reshape((H_patch*W_patch,5))
    sp = str(feature_path).split('/'); id =sp[-1].replace('.pth.npy','');
...
def _extract_bbox(
    ...
    sp = str(feature_path).split('/'); image_id =sp[-1].replace('.pth.npy','');
    P=16
    # Sizes
