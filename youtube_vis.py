...
base_dir='D:/Dataset/Vis2019/train/'
dataset_json = 'C:/Users/Farnoosh/Desktop/fd/youtube_vis_train.json'
dataset,meta_info,seqs =  parse_generic_video_dataset(base_dir, dataset_json)

def show_vis(msk,rgb):
    a = np.expand_dims(rgb[:,:,0],2)
    b = np.expand_dims(rgb[:,:,0],2)
    c = np.expand_dims(msk,2)
    out = np.concatenate((a,b,c*255),axis=-1)
    plt.imshow(out)
