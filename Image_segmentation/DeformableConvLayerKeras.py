...
class DeformableConv2D(tf.keras.layers.Conv2D):
    ...
    def deformable_conv(self, input, name, batch_size, filters, kernel_size):
        ...
        for grid in [grid_x, grid_y]:
            ...
            patched_grid = tf.compat.v1.extract_image_patches(grid,
        ...
        for offset in [off_x, off_y]:
            patched_offset = tf.compat.v1.extract_image_patches(offset,
                                                            ksizes = (1,) + kernel_size + (1,),
                                                            strides = [1, 1, 1, 1],
                                                            rates = [1, 1, 1, 1],
                                                            padding = 'SAME')
    ...
    def _inference_grid_offset(self, input_images):
        ...
        input_tensor = tf.keras.layers.Input(dtype=tf.float32, shape=[h, w, c])
        #input_tensor = tf.compat.v1.placeholder(tf.float32, [None, h, w, c])
