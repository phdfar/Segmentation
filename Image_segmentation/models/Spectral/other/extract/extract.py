...
def extract_features(name,save=False,outputfile='',images_root=''):
    ...
    for i, (images, files, indices) in enumerate(dataloader):
        ...
        eigenvectors,eigenvalues=extract_eig(filenames[0],output_dict,K,which_matrix,which_features,normalize,lapnorm,which_color_matrix,threshold_at_zero,image_downsample_factor,image_color_lambda)
        
        eigenvectors = eigenvectors.T.reshape((H_patch,W_patch,K))
        
        if save==True:
            out_name=name.replace('.jpg','npy')
            outfile=outputfile+'/'+out_name
            np.save(outfile, eigenvectors)

        
        e1= NormalizeData(np.expand_dims(eigenvectors[:,:,1],2))
        e2= NormalizeData(np.expand_dims(eigenvectors[:,:,2],2))

        img = np.concatenate((e1,e2),axis=-1)
        
        return img
    

def extract_eig(filenames,data_dict,K,which_matrix,which_features,normalize,lapnorm,which_color_matrix,threshold_at_zero,image_downsample_factor,image_color_lambda):
    ...
    # Eigenvectors of matting laplacian matrix
        ...
        if image_color_lambda > 0:
            ...
            image_lr = Image.open(filenames).resize((W_pad_lr, H_pad_lr), Image.BILINEAR)
    ...
    return eigenvectors.numpy(),eigenvalues.numpy()
